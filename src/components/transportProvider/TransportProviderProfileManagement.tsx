...
interface TransportProviderProfileManagementProps {
  transportProviderId: string;
  transportProviderData: any;
  onProfileUpdate: (updatedData: any) => void;
}

const TransportProviderProfileManagement: React.FC<
  TransportProviderProfileManagementProps
> = ({ transportProviderId, transportProviderData, onProfileUpdate }) => {
  ...
  const editProfileSchema = transportProviderFormSchema.omit({ phone: true });

  const form = useForm<Omit<TransportProviderFormValues, "phone">>({
    resolver: zodResolver(editProfileSchema),
    defaultValues: {
      name: transportProviderData?.name || "",
      email: transportProviderData?.email || "",
      country: transportProviderData?.country || "India",
      country_code: transportProviderData?.country_code || "+91",
      company_name: transportProviderData?.company_name || "",
      city: transportProviderData?.city || "",
      pincode: transportProviderData?.pincode || "",
      state: transportProviderData?.state || "",
      district: transportProviderData?.district || "",
      transport_types: transportProviderData?.transport_types || [],
      service_area: transportProviderData?.service_area || "",
      exim_code: transportProviderData?.exim_code || "",
      pan_no: transportProviderData?.pan_no || "",
      biz_gst: transportProviderData?.biz_gst || "",
      additional_info: transportProviderData?.additional_info || "",
    },
  });
  ...
  useEffect(() => {
    if (
      ...
      transportProviderData?.country === "India"
    ) {
      setAvailableDistricts(getDistrictsByState(transportProviderData.state));
    }
  ...
  const handleSave = async (
    values: Omit<TransportProviderFormValues, "phone">,
  ) => {
    setIsLoading(true);
    try {
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex">
                    <div className="bg-muted flex items-center justify-center px-3 border border-r-0 rounded-l-md">
                      {transportProviderData?.country_code || "+91"}
                    </div>
                    <Input
                      className="rounded-l-none bg-gray-50"
                      value={transportProviderData?.phone || ""}
                      disabled
                    />
                  </div>
...
export default TransportProviderProfileManagement;
